  ...
  <div class="contact-container">
    ...
    <b-form @submit="onSubmit" @reset="onReset" v-if="true" v-scroll-reveal>
      <b-form-group id="input-group-1" label="姓名" label-for="input-1">
        <b-form-input id="input-1" v-model="form.name" placeholder="请输入姓名"></b-form-input>
        <div v-if="errors.name" class="err-msg">{{ errors.name }}</div>
      </b-form-group>
      <b-form-group id="input-group-2" label="手机号码" label-for="input-2" description>
        <b-form-input id="input-2" v-model="form.mobile" placeholder="请输入手机号码"></b-form-input>
        <div v-if="errors.mobile" class="err-msg">{{ errors.mobile }}</div>
      </b-form-group>
      <b-form-group id="textarea-1" label="留言" label-for="input-2" description>
        <b-form-textarea
          id="textarea"
...
export default {
  ...
  methods: {
    onSubmit(evt) {
      ...
      if (this.checkForm()) {
        axios
          .post("http://www.woyaotest.com:3000/sms", {
            name: this.form.name,
